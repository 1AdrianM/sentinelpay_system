...
import com.github.sentinel.pay.application.usecases.GetRiskProfileByAccountIdUseCase;
...
public class GetRiskProfileByAccountIdInteractor implements GetRiskProfileByAccountIdUseCase {
    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(GetRiskProfileByAccountIdInteractor.class);
   ...
   private final FraudIncidentRepository fraudIncidentRepository;
    ...
    public RiskProfileDto execute(String accountId) {
        logger.atInfo().log("Starting risk profile retrieval process for account ID: {}", accountId);
        if (accountId.isEmpty() || accountId.isBlank()){
            logger.atError().log("Account ID parameter is empty or blank");
    ...
    AccountId accountID =  new AccountId(UUID.fromString(accountId));
        AccountRiskProfile riskProfile =riskProfileRepository.findByAccountId(accountID).orElseThrow(()->{
            logger.atError().log("Risk profile not found for account ID: {}", accountID.id());
            throw new RuntimeException("Risk profile not found");
        });
        logger.atInfo().log("Retrieved risk profile for account ID: {}", accountID.id());

      List<FraudIncident> incidentList= fraudIncidentRepository.findAllByAccountId(accountID, PageRequest.of(0,10));
      logger.atInfo().log("Retrieved {} fraud incidents for account ID: {}", incidentList.size(), accountID.id());
